...
function MatchedResult({ image }) {
  ...
  } else itemName = image.name;

  const handleClick = () => {
    setFill(fill === 'currentColor' ? 'gold' : 'currentColor');
    setPath(path === emptyStarPath ? filledStarPath : emptyStarPath);
  };

  return (
    <div className='matchedResult'>
      <a href={image.hostPageUrl} target="_blank"><img src={image.contentUrl} alt={image.name} />
        <br></br>{itemName}
      </a>
      <br></br>
      <div  className="star">
